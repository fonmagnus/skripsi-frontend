<template>
  <div class="flex flex-column">
    ...
    <latex v-if="answer.question.desc" :content="readMD(answer.question.desc)" class="ml-3 mt-3 html-container"></latex>

    <div class="flex centered-items start-content ml-3 mt-3">
      <vs-radio 
        v-for="(c, i) in answer.question.choice_options.split('\r\n')"
        :key="i"
        :val="i+1"
        class="mx-3"
        :color="answer.question.max_score === answer.score ? 'success' : 'danger'"
        disabled
        v-model="answer.body">
        {{c}}
      </vs-radio>
    ...
    <span v-if="answer.score !== answer.question.max_score" class="ml-3 mt-4">Correct Answer : </span>
    <div v-if="answer.score !== answer.question.max_score" class="flex centered-items start-content ml-3 mt-3">
      <vs-radio 
        v-for="(c, i) in answer.question.choice_options.split('\r\n')"
        :key="i"
        :val="i+1"
