<template>
  <div
    ...
    :class="$vuetify.breakpoint.smAndDown ? 'mt-5 mx-3 my-3' : ''"
  >
    <vs-select
    ...
    <div class="flex mb-2">
      ...
      <vs-radio
        ...
        v-model="picked"
        val="2"
        >All Submissions</vs-radio
      >
    </div>
    <vs-table v-model="selected" striped v-if="problemset.type === 'MIXED'">
      <template #thead>
        <vs-tr>
          <vs-th> ID </vs-th>
          <vs-th> Email </vs-th>
      ...
      <template #tbody>
        <vs-tr
          ...
          v-for="(tr, i) in $vs.getPage(
            $vs.getSearch(submissions, search),
            page,
            maxDisplayItems
          )"
          :data="tr"
      ...
      <template #footer>
        <vs-pagination
          v-model="page"
          :length="
            $vs.getLength($vs.getSearch(submissions, search), maxDisplayItems)
          "
        />
      </template>
    ...
    >
      ...
      <template #thead>
        <vs-tr>
          ...
          <vs-th> Problem </vs-th>
          <vs-th> Verdict </vs-th>
          <vs-th v-if="problemset.type === 'INTERNAL_CODING'"> Score </vs-th>
      ...
      <template #tbody>
        ...
        >
          ...
          <vs-td>
            <span v-if="problemset.type === 'INTERNAL_CODING'">{{
              tr.coding_problem_slug
            }}</span>
            <span v-else-if="problemset.type === 'FULL_CODING'"
              >{{ tr.oj_name }} {{ tr.oj_problem_code }}</span
            >
          </vs-td>
          ...
          <vs-td
            v-if="problemset.type === 'INTERNAL_CODING'"
            :style="formatClassWithoutStatus(tr.score)"
            class="verdict"
          >
            {{ tr.score }}
          </vs-td>
