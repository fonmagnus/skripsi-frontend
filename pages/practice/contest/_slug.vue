<template>
  <div v-if="isAllowed" class="flex items-center justify-center flex-col pt-5">
    <div class="mt-1">
      <ContestLayout :problems="problems" />
    </div>
  </div>
  <div v-else class="flex items-center justify-center flex-col fullscreen pa-3">
    ...
    <vs-dialog
      prevent-close
      width="750px"
      v-model="isShowingTeamFormDialog"
      class="pa-3"
    >
      <template #header>
        <h4 class="mt-3">Select or Create a Team</h4>
      </template>
      <div class="flex flex-col">
        <div class="flex justify-end">
          <vs-button
            v-if="!isCreatingNewTeam"
            class="button mb-3"
            transparent
            @click="createNewTeam"
          >
        ...
        <div
          v-if="isCreatingNewTeam"
          class="flex flex-col items-start justify-start mb-5 ml-3"
        >
        ...
        <vs-table v-if="!isCreatingNewTeam">
          ...
          <template #tbody>
            <vs-tr
              v-for="(team, i) in $vs.getPage(
                $vs.getSearch(myTeams, searchTeam),
                teamPage,
                teamMax
              )"
              :key="i"
          ...
          <template #footer>
            <vs-pagination
              v-model="teamPage"
              :length="$vs.getLength($vs.getSearch(myTeams), teamMax)"
            />
          </template>
        </vs-table>
        <div class="flex items-center justify-center mt-5">
          <vs-button
            class="button"
            transparent
            @click="isShowingTeamFormDialog = false"
            color="dark"
          >
            Batal
          </vs-button>
...
export default {
  ...
  data() {
    return {
      ...
      isAllowed: true,
      forbiddenMessage:
        "Kamu dilarang mengakses halaman ini. Jika kamu sudah mendaftar sebagai partisipan, hubungi admin untuk mendapatkan approval",
        ...
        if (this.$auth.user.role !== "Admin") {
          ...
          if (now < startTime) {
            ...
            this.forbiddenMessage =
              "Kontes ini belum dimulai, kamu belum bisa mengaksesnya";
          }

          if (this.problemset.enable_team_contest) {
            this.$services.problem
              .getMyTeamInProblemset(
              ...
              .catch((err) => {
                this.isAllowed = false;
                this.forbiddenMessage =
                  "Ini adalah kontes tim, kamu harus mendaftarkan dulu tim mu sebelumnya";
              });
          }
        }
      });
